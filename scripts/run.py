...
def assert_dead_daemons():
    ...
    """
    check_monitor_cmd = f"systemctl --user --type=service --all --state=active | grep -e ^{daemon.name}@monitor"
    check_node_cmd = f"systemctl --user --type=service --all --state=active | grep -e ^{daemon.name}@node"
